...
'----------------------------------------------------------------------------------------------------------------------------------------------------------
' 컬럼추가 :
' type_code 1 :  오른쪽으로 추가
'----------------------------------------------------------------------------------------------------------------------------------------------------------
Sub InsertCell(tarShtName As Variant, tarRange As Variant, type_code As Variant)

'    ActiveWorkbook.Worksheets("Code").Columns("A").Select
'    Columns("A:A").Select
'    Selection.Insert Shift:=xlToRight
    
    Const INSERT_COLUM As Integer = 1

    Select Case type_code
        
        Case INSERT_COLUM
            Workbooks("KM_SUM").Worksheets(tarShtName).Activate
            With ActiveSheet.Columns(tarRange)
                .Select
